...
public class TeamPaintFalltrapCommand extends Argument {

    private final Map<UUID, TeamFalltrapCommand.PlayerClaim> claimMap;
    private final Cooldown cooldown;
    private final ItemStack claimWand;
    private static final int MAX_BLOCK_SELECTION = 35; // Límite de bloques seleccionados

    public TeamPaintFalltrapCommand(CommandManager manager) {
        super(manager, Collections.singletonList("paintfalltrap"));
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!pt.checkRole(player, Role.CO_LEADER)) {
            sendMessage(sender, Config.INSUFFICIENT_ROLE.replace("%role%", Role.CO_LEADER.getName()));
            return;
        }
        ...
        claimMap.put(player.getUniqueId(), new TeamFalltrapCommand.PlayerClaim());
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        PlayerTeam pt = getInstance().getTeamManager().getByPlayer(player.getUniqueId());
        TeamFalltrapCommand.PlayerClaim claim = claimMap.get(player.getUniqueId());
        ...
        if ((e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) && player.isSneaking()) {
            ...
            int selectedBlocks = countSelectedBlocks(claim);
            if (selectedBlocks > MAX_BLOCK_SELECTION) {
                sendMessage(player, "Solo puedes seleccionar un máximo de " + MAX_BLOCK_SELECTION + " bloques.");
                return;
            }
        ...
        if (action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (location1 != null && location1.getBlockX() == blockClick.getBlockX() && location1.getBlockZ() == blockClick.getBlockZ()) return;
        ...
        if (action == Action.LEFT_CLICK_BLOCK) {
            ...
            if (location2 != null && location2.getBlockX() == blockClick.getBlockX() && location2.getBlockZ() == blockClick.getBlockZ()) return;
    ...
    private int countSelectedBlocks(TeamFalltrapCommand.PlayerClaim claim) {
        return Math.abs(claim.getLocation1().getBlockX() - claim.getLocation2().getBlockX()) *
                Math.abs(claim.getLocation1().getBlockZ() - claim.getLocation2().getBlockZ());
    }
