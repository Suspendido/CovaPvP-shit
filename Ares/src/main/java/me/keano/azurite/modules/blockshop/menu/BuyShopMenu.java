...
public class BuyShopMenu extends Menu {
    ...
    public BuyShopMenu(MenuManager manager, Player player) {
        super(
                manager,
                player,
                manager.getBlockshopConfig().getString("BUY_SHOP_MENU.TITLE"),
                manager.getBlockshopConfig().getInt("BUY_SHOP_MENU.SIZE"),
                true
        );
    }
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        all.putAll(getInstance().getBlockshopManager().getButtons(player, "BUY_SHOP_MENU.BUTTONS"));
        return all;
    }
}