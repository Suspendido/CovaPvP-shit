...
public class SellShopMenu extends Menu {
    ...
    public SellShopMenu(MenuManager manager, Player player) {
        super(
                manager,
                player,
                manager.getBlockshopConfig().getString("SELL_SHOP_MENU.TITLE"),
                manager.getBlockshopConfig().getInt("SELL_SHOP_MENU.SIZE"),
                true
        );
    }
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        all.putAll(getInstance().getBlockshopManager().getButtons(player, "SELL_SHOP_MENU.BUTTONS"));
        return all;
    }
}